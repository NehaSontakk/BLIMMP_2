...
Combined recall & precision vs HMMsearch (E < 1e-5) stable KOs at 0% removal.

Ground truth: HMMsearch stable KOs at 0% removal (present in ≥2 of 3 replicates).
This is the most permissive baseline — raw HMM hits, no score filtering.

  Recall    = |tool KOs ∩ hmm_stable| / |hmm_stable|
              (fraction of HMM's 0%-removal set that this tool detects)
  Precision = |tool KOs ∩ hmm_stable| / |tool KOs|
              (fraction of tool's calls that are in the HMM baseline)
...
Writes: PLOTS/ko_hmmsearch_recovery.pdf/.png
"""
...
removal_levels_plot = [r for r in removal_levels if r < 100]   # truncate at 90%
print(f"Organisms: {organisms}")
print(f"Removal levels: {removal_levels}  (plotting up to {removal_levels_plot[-1]}%)")
...
# ---------------------------------------------------------------------------
# Step 1: HMMsearch stable KO set per organism at 0% removal
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Step 2: recall and precision per (tool, organism, removal_pct, replicate)
# ---------------------------------------------------------------------------
rows = []
...
print("\nRecall and Precision at 0% removal (HMMsearch ground truth):")
...
METRIC_CFG = [
    ("rec_mean", "rec_sd",
     "Recall\n(HMMsearch KOs recovered / HMMsearch stable, mean ± SD)"),
    ("pre_mean", "pre_sd",
     "Precision\n(HMMsearch KOs recovered / tool KOs, mean ± SD)"),
]
...
for row_i, (mean_col, sd_col, ylabel) in enumerate(METRIC_CFG):
    for col_i, org in enumerate(organisms):
        ...
        if row_i == 0:
            ...
            ax.text(0.97, 0.04, f"HMMsearch n={n_gt:,}",
...
fig.suptitle(
    "Recall and Precision vs HMMsearch (E < 1e-5) stable KOs at 0% removal",
...
fig.text(
    0.5, -0.02,
    "BLIMMP thresholds (ko_probability): direct > 0.01, neighbor > 0.01, +sub > 0.01   "
    "|   Ground truth = HMMsearch (E < 1e-5) stable KOs at 0% removal (present in ≥2 of 3 replicates)",
...
fig.savefig(OUT_DIR / "ko_hmmsearch_recovery.png", dpi=300,
            bbox_inches="tight", facecolor=SURFACE)
fig.savefig(OUT_DIR / "ko_hmmsearch_recovery.pdf",
            bbox_inches="tight", facecolor=SURFACE)
print(f"\nSaved: {OUT_DIR}/ko_hmmsearch_recovery.{{png,pdf}}")
plt.close("all")
